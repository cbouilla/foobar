...
struct context_t_v2 {
        ...
        /**** output ****/
        struct task_result_t_v2 *result;
};
...
void process_slice_v3(struct context_t *self, const struct slice_t *slice, 
                                                                bool verbose);
void process_slice_v4(struct context_t_v2 *self, const struct slice_t *slice, u32 task_index[2], bool verbose);
...
u64 subjoin_v2(struct slice_ctx_t_v2 *ctx, u32 T, struct scattered_t *partitions);
...
void prepare_side_v2(struct context_t_v2 *self, u32 k, bool verbose)
{
        u32 T = self->T_part;
        u32 n = self->n[k];
        u32 fan_out = 1 << self->p;
...
void process_slice_v4(struct context_t_v2 *self, const struct slice_t *slice, u32 task_index[2],
                                                               					 bool verbose)
{
        double start = wtime();
        struct slice_ctx_t_v2 ctx = { .slice = slice };
        ctx.result = result_init_v2();
        ctx.H = hashtable_build(slice->CM, 0, slice->n);
        ...
        #pragma omp parallel reduction(+:probes, instr, cycles) num_threads(self->T_subj)
        {
                ...
                for (u32 i = 0; i < fan_out; i++) {
                        ...
                        probes += subjoin_v2(&ctx, T, scattered);
                }
                if (false) {
                        counters[0] = counters[1] = 0;
                        rc = PAPI_read_counters(counters, 2);
        ...
        struct solution_t_v2 *loc = ctx.result->solutions;
	struct solution_t_v2 solution;
	//u64 (*loc)[3] = ctx.result->solutions;
        u32 n_sols = ctx.result->size;
        for (u32 i = 0; i < n_sols; i++) {
		...
		solution.task_index[1] = task_index[1];
                report_solution_v2(self->result, solution);
        }

        hashtable_free(ctx.H);
        result_free_v2(ctx.result);
        ...
        for (u32 t = 0; t < T; t++) {
                ...
                for (u32 i = 0; i < nB; i++) {
						...
						solution.task_index[1] = 0;						
                                                report_solution_v2(ctx->result, solution);
					}
                                        emitted++;
...
struct task_result_t_v2 * iterated_joux_task_v4(struct jtask_t *task, u32 task_index[2])
{
        ...
        static const u32 p = 10;                             // hardcodé !
        //struct task_result_t *result = result_init();
	struct task_result_t_v2 *result = result_init_v2();
        double start = wtime();
        //struct context_t self;
	struct context_t_v2 self;
        for (u32 k = 0; k < 2; k++) {
                self.n[k] = task->n[k];
                self.L[k] = task->L[k];
        }
        ...
        self.result = result;
        for (u32 k = 0; k < 2; k++)
                prepare_side_v2(&self, k, task_verbose);
        self.volume = 0;
        self.gemm_usec = 0;
        self.gemm_instr = 0;
        self.gemm_cycles = 0;
        ...
        while (1) {
                //process_slice_v3(&self, slice, slice_verbose);
		process_slice_v4(&self, slice, task_index, slice_verbose);
                i++;
                u32 n = slice->n;
                u8 *ptr = (u8 *) slice;
                ptr += sizeof(struct slice_t) + sizeof(u64) * n;
